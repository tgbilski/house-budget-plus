-- Insert the Community Marketplace blog post using the admin user
DO $$
DECLARE
  admin_user_id uuid;
BEGIN
  -- Get the admin user ID from profiles/user_roles
  SELECT p.user_id INTO admin_user_id 
  FROM profiles p 
  JOIN user_roles ur ON p.user_id = ur.user_id 
  WHERE p.email = 'tgbilski@gmail.com' AND ur.role = 'admin' 
  LIMIT 1;
  
  -- Insert the blog post if admin found and post doesn't exist
