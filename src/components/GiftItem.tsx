...
export function GiftItem({ item, onSave, onDelete, isNew = false, onCancel }: GiftItemProps) {
    ...
    return (
      <div className="border border-gray-200 rounded-lg p-4 space-y-3 bg-gray-50">
        <div className="space-y-2">
            <div>
              <Label htmlFor={`gift-idea-${item.id}`}>Gift Idea</Label>
              <Input id={`gift-idea-${item.id}`} value={itemData.gift_idea} onChange={(e) => handleInputChange('gift_idea', e.target.value)} placeholder="A great gift idea..." />
            </div>
            <div className="grid grid-cols-2 gap-2">
              <div>
                <Label htmlFor={`price-${item.id}`}>Price</Label>
                <Input id={`price-${item.id}`} type="number" value={itemData.price} onChange={(e) => handleInputChange('price', e.target.value)} placeholder="0.00" />
              </div>
              <div>
                <Label htmlFor={`url-${item.id}`}>URL</Label>
                <Input id={`url-${item.id}`} value={itemData.url} onChange={(e) => handleInputChange('url', e.target.value)} placeholder="https://example.com" />
              </div>
            </div>
          </div>
  ...
  return (
    <div className="border border-gray-200 rounded-lg p-3 bg-white flex items-center justify-between group shadow-sm">
      <div className="flex-1 min-w-0">
        <h4 className="font-medium text-sm truncate text-gray-900">{itemData.gift_idea || 'Untitled Gift'}</h4>
        <div className="flex items-center gap-4 mt-1 text-xs text-gray-500">
          {itemData.price ? <span className="font-semibold text-green-600">${Number(itemData.price).toFixed(2)}</span> : null}
